...
router.post("/", async (req, res) => {
  try {
    ...
    setImmediate(async () => {
      try {
        await notifyNearbyUsers("offer_help", inserted, { excludeUserId: user_id });
      } catch {}
    });
...
router.get("/", async (_req, res) => {
  try {
    const db = getDB();
    const docs = await db.collection("offer_help").find({}).sort({ timestamp: -1 }).limit(100).toArray();
...
router.get("/near", async (req, res) => {
  try {
    ...
    const db = getDB();
    const results = await db.collection("offer_help").find({
      location: {
        $nearSphere: {
...
router.patch("/:id/confirm", async (req, res) => {
  try {
    ...
    setImmediate(() => notifyFollowersOfUpdate("offer_help", id, user_id, "confirm", "An offer was confirmed."));
...
router.patch("/:id/dispute", async (req, res) => {
  try {
    ...
    await coll.updateOne(query, update);
    setImmediate(() => notifyFollowersOfUpdate("offer_help", id, user_id, "dispute", "An offer was disputed."));
...
router.patch("/:id/resolve", async (req, res) => {
  try {
    ...
    setImmediate(() => notifyFollowersOfUpdate("offer_help", id, null, "resolve", "A followed offer was resolved."));
...
router.patch("/:id/follow", async (req, res) => {
  try {
    ...
    if (!alreadyFollowing)
      setImmediate(() => notifyFollowersOfUpdate("offer_help", id, user_id, "follow", "A post you follow has a new follower."));
...
router.post("/:id/comments", async (req, res) => {
  try {
    ...
    const comment = { offer_id: offerDoc._id, user_id: user_id || null, text, createdAt: new Date() };
    const result = await comments.insertOne(comment);

    setImmediate(() => notifyFollowersOfUpdate("offer_help", id, user_id, "comment", text));
...
router.get("/:id/comments", async (req, res) => {
  try {
    const db = getDB();
    const id = req.params.id;
    const filter = /^[0-9a-fA-F]{24}$/.test(id) ? { offer_id: new ObjectId(id) } : { offer_id: id };
    const docs = await db.collection("offer_comments").find(filter).sort({ createdAt: 1 }).toArray();
